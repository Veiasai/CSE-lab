...
check_network() {
  ...
  if ! (grep app /etc/hosts >/dev/null); then
  ...
  if ! (grep data1 /etc/hosts >/dev/null); then
    echo "You didn't setup /etc/hosts correctly"
    exit 1
  fi
  if ! (grep data2 /etc/hosts >/dev/null); then
    echo "You didn't setup /etc/hosts correctly"
    exit 1
  fi
  ...
  myip=$(grep $myname /etc/hosts | grep -o -E "[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+")
...
if [ -f app_public_ip ]; then
  scp test-lab4-common.sh test-lab4-part0.sh cse@$(cat app_public_ip): >/dev/null 2>&1
  remote_grade /home/cse/test-lab4-part0.sh
  ret=$?
  ssh cse@$(cat app_public_ip) rm test-lab4-common.sh test-lab4-part0.sh >/dev/null 2>&1
  exit $ret
fi
